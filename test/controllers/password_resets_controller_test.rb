...
class PasswordResetsControllerTest < ActionDispatch::IntegrationTest 
    ...
    test "#create" do 
        assert_emails 1 do 
            post password_reset_path, params: { email: users(:one).email }
        end
    ...
    test "#updated resets the password with a valid token and valid params" do 
        user = users(:one)
    ...
    test "#update fails if password and password confirmation don't match" do
        ...
        patch password_reset_path, params: {
            password_reset_token: token, 
            password_reset: { password: "newpassword", password_confirmation: "newpassword2"}
        }
